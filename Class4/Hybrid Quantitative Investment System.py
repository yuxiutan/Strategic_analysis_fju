...
class FuturePredictor:
    ...
    def predict_future_days(self, data, n_days=5):
        ...
        # Estimate future price range based on ATR
        estimated_changes = []
        for i in range(n_days):
            # Use probability to estimate direction and magnitude
            if latest_probability > 0.5:
                # Bullish bias
                change_pct = (latest_probability - 0.5) * 2 * 0.02 # Max 2% per day
            else:
                # Bearish bias
                change_pct = (latest_probability - 0.5) * 2 * 0.02 # Negative change
